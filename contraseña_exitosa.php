...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($usuario) || empty($contrasena)) {
        $error = "Por favor, ingresa el nombre de usuario y la contraseña.";
    } else {
        ...
        if (mysqli_num_rows($result) === 1) {
                ...
                if ($row['cambio_contrasena'] == 1) {
                    ...
                    switch ($rol) {
                        case 'admin':
                            ...
                            break;
                        case 'digitador':
                            exit(header("Location: digitador.php"));
                            break;
                        case 'consultor':
            ...
            $error = "Nombre de usuario incorrecto. Por favor, inténtalo nuevamente.";
        }
    }
    mysqli_close($conn);
}
...
<body>
    <div id="box-login">
        <div id="text-center">Bienvenidos a La Plataforma De Registro De Votantes</div>
        <div id="text-center">Inicie Sección Con Su Usuario Y Contraseña</div>
        <br>
        <div id="text-center">Cambio de Contraseña Exitosa</div>
        <h2>Usa tu Nueva Contraseña</h2>
        <?php if (!empty($error)) { ?>
